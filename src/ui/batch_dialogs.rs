    ...
    if app.show_batch_delete_confirm {
        egui::Window::new("🧹 Remove Black Images")
            ...
            .show(ctx, |ui| {
                ...
                ui.label(format!("Total images: {}", app.dataset.get_image_files().len()));
...
pub fn render_batch_progress(app: &mut DatasetCleanerApp, ctx: &egui::Context) {
    if app.batch_processing || (app.batch_stats.is_some() && !app.show_batch_delete_confirm) {
        egui::Window::new(if app.batch_processing { "⏳ Processing..." } else { "✓ Processing Complete" })
            .collapsible(false)
            .resizable(false)
            .anchor(egui::Align2::CENTER_CENTER, [0.0, 0.0])
            .show(ctx, |ui| {
                if let Some(stats) = &app.batch_stats {
                    if app.batch_processing {
                        ui.label(format!("Scanning images: {}/{}", 
                        ...
                        let retention_rate = if stats.total_scanned > 0 {
                            ((stats.total_scanned - stats.total_deleted) as f32 / stats.total_scanned as f32) * 100.0
                        } else {
                            0.0
                        };
